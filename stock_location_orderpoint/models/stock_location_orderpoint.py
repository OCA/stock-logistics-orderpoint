...
class StockLocationOrderpoint(models.Model):
    ...
    def _get_covering_replenishment_moves(self, products):
        ...
        location for the given products, generated by a lower-priority
        orderpoint sharing that location."""
        self.ensure_one()
        if not products:
            return self.env["stock.move"]
        return self.env["stock.move"].search(
            self._get_covering_replenishment_moves_domain(products)
        )

    def _get_covering_replenishment_moves_domain(self, products):
        """Return the domain to search for not-yet-started replenishment
        moves, generated by a lower-priority orderpoint, that are already
        ...
        return expression.AND(
            [
                [
                    ...
                    ("state", "not in", ("waiting", "done", "cancel")),
                    ("location_orderpoint_id.priority", "<", self.priority or "0"),
                ],
                self._get_pending_replenishment_sourcing_process_domain(products),
            ]
        )
    ...
    def _cancel_fulfilled_replenishment_moves(self, products):
        ...
        Recomputes the demand for all given products in a single call,
        excluding each not-yet-started replenishment move's own quantity
        from the incoming forecast, and cancels the moves of the products
        that no longer have any remaining demand. Moves of products that
        still have some remaining demand are left untouched.

        TODO: We don't handle the case where the move is partially fulfilled
        IOW when the move's quantity is bigger than the remaining demand.
        This case could be hard to handle correctly since the qty to procure
        is not enforced as the demand but provided by the orderpoint's
        replenishment computer via the `_compute_replenishment_qty` method.

        :param products: product.product recordset to check
        :return: recordset of cancelled stock.move
        """
        self.ensure_one()
        moves = self._get_not_started_replenishment_moves(products)
        if not moves:
            return moves
        demand_data = self._get_remaining_demand(moves)
        moves_to_cancel = moves.filtered(lambda m: not demand_data.get(m.product_id.id))
        if not moves_to_cancel:
            return self.env["stock.move"]
        return self._do_cancel_fulfilled_replenishment_moves(moves_to_cancel)
    ...
    def _get_pending_replenishment_sourcing_process_domain(self, products):
        """Return the domain to search for pending processes responsible of
        sourcing replenishment moves.

        The ``products`` parameter is not used by the base implementation:
        it is provided so that overrides can restrict or extend the domain
        based on the products being replenished.

        This method is an extension point that can be overridden for example
        when used in combination with mrp (Manufacturing Resource Planning).
        In such cases, this method can be overridden to include additional
        domains that link replenishment moves to manufacturing orders as
        sourcing processes.

        i.e.:
            def _get_pending_replenishment_sourcing_process_domain(self, products):
                domain = super()._get_pending_replenishment_sourcing_process_domain(
                    products
                )
                return expression.OR([domain, [("workorder_id", "in", ...)]])

        :param products: product.product recordset the domain is built for
        :return: domain (list of tuples) usable to search stock.move records
        """
        self.ensure_one()
        not_started_picking_query = self.env["stock.picking"]._search(
            self._get_not_started_replenishment_pickings_domain(),
            order="id",
        )
        return [
            ("picking_id", "in", not_started_picking_query),
        ]
    ...
    def _get_not_started_replenishment_pickings_domain(self):
        """Return the domain part related to pickings to search
        for replenishment moves generated by this orderpoint"""
        self.ensure_one()
        return [
            ("printed", "=", False),
            ("state", "not in", ("waiting", "done", "cancel")),
    ...
    def _get_remaining_demand(self, moves):
        """Recompute the current demand for the products of the given moves,
        excluding these moves' own quantity from the incoming forecast.
        Otherwise a not-yet-started replenishment move always appears to
        fulfill the very demand it was created for.

        :param moves: stock.move recordset to exclude from the forecast
